...
from src.loss import MS_SSIM
from src.pytorch_mssim import MSSSIM
from torch.autograd import Variable
from src.dataset import DatasetFromFolder, getCoverExample, getSecretExample, DatasetFromFolder2
# from models.HidingUNet import UnetGenerator
# from models.RevealNet import RevealNet
from src.model import HidingNet, RevealNet,  Discriminator
from src.XuNet import XuNet
import src.utils
from src.ssim import SSIM
from src.utils import to_np, plot_train_result, plot_result, plotCoverSecret, ssim_covers
from src.utils import plotCovers, plotSecrets
...
def print_network(name, net):
    ...
    print(name, 'Total number of parameters: {}'.format(num_params))
...
test_data = DatasetFromFolder2(data_dir, subfolder='test.txt', transform=transform,
...
def train(epoch):
    ...
    for idx, (secret, cover, target) in enumerate(train_data_loader, 1):
        ...
        disloss1 = BCE_loss(cls1, Variable(torch.ones(cls1.size()) * random.uniform(0.8, 1.2)))
        ...
        if idx % 5 == 0:
            ...
            discover = BCE_loss(cls_cover, Variable(torch.ones(cls_cover.size()) * random.uniform(0.8, 1.2)))
            disstego = BCE_loss(cls_stego, Variable(torch.zeros(cls_stego.size()) * random.uniform(0., 0.2)))
